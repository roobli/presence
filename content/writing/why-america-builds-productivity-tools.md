...
```mermaid
flowchart TD
  A[Expensive labor — time is dear] --> B[Firms and people will pay for software]
  I[Buyer-side engineers; employee cards] --> B
  B --> C[Standard SaaS earns stable subscription]
  H[English makes the product global on day one] --> C
  C --> D[Capital richly values recurring revenue]
  D --> E[Global founders raise and list in the U.S.]
  E -->|more products of the same shape| C
  C -->|marketing budgets| F[Tool companies sponsor creators]
  F --> G[Efficiency and self-improvement narratives amplify]
  G -->|feeds willingness to pay| B
```
